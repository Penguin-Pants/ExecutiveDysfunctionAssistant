...
| **FR64** | v1 | Panic clear stops capture and leaves the session in `WIPED`: buffers cleared, overlay empty, devices still open and preflight still valid, resumable with one click in ≤1 s. It does not end the session, and it does not leave capture running. | Panic-clear mid-session; assert capture stopped, assert resume needs no preflight re-run and completes within 1 s. (D-U5.) |
