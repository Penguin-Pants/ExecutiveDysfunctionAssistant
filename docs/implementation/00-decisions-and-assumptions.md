...
| **D-U7** | Overlay palette | **The overlay is a neutral translucent gray panel, not PRISM's plum** — stated by the user as overriding PRISM. PRISM still governs the overlay's typography, 20px radius, spacing ladder and semantic rail colours, and governs the app chrome entirely. | A saturated surface tints the video behind it; a neutral translucent one does not. Scoped to the overlay so the rest of the product stays on-brand. **Sub-question D-U7a is open:** dark-neutral (FR11's own wording) vs genuinely light gray. Both rendered in the mockup; A is the working default. |
