...
| **T9.6a** Real dependency construction | FR43, D-U3 | `_build_application` constructs the embedder, model client, cipher and active note set. **Deliberately unimplemented**, on two blockers — not three. ~~FR43's active-note-set selection~~ **is done**: T3.8 writes the id and `ui.editor.load_active_set(root, default_settings())` reads it. What remains is the **no-API-key policy** (a product decision belonging with T9.3) and the two dependencies themselves — a real embedding model, blocked in this container by network policy rather than by platform, and DPAPI, which is genuinely Windows-only. Raising is the honest state. |
