...
| **T9.2b** Open the settings dialog from the app | FR52, FR37 | A main window or menu constructs `SettingsDialog`, feeds `on_switch` to `SessionManager.set_switch`, and passes the result to `Application.apply_settings`. *(Split out 2026-08-14: the pieces exist and are tested; nothing opens them, because there is no main window yet — same shape as T9.1a.)* |
